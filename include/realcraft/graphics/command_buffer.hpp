...
public:
    ...
    virtual void begin_render_pass(const RenderPassDesc& desc, Texture* color_attachment,
                                   Texture* depth_attachment, const ClearValue& color_clear,
                                   const ClearValue& depth_clear) = 0;
    ...
    virtual void bind_uniform_buffer(uint32_t binding, const Buffer* buffer, size_t offset = 0,
                                     size_t size = 0) = 0;

    // Bind texture with sampler to a binding slot
    virtual void bind_texture(uint32_t binding, const Texture* texture,
                              const Sampler* sampler) = 0;

    // Bind storage buffer to a binding slot
    virtual void bind_storage_buffer(uint32_t binding, const Buffer* buffer, size_t offset = 0,
                                     size_t size = 0) = 0;
    ...
    virtual void push_constants(ShaderStage stage, uint32_t offset, uint32_t size,
                                const void* data) = 0;
    ...
    virtual void copy_buffer(const Buffer* src, Buffer* dst, size_t src_offset, size_t dst_offset,
                             size_t size) = 0;
